...
serve(async (req) => {
  ...
  try {
    console.log('📨 Received GUEST checkout request:', req.method);

    // Get and TRIM environment variables (removes hidden whitespace/newlines)
    const MERCHANT_ID = Deno.env.get('CCAVENUE_MERCHANT_ID')?.trim();
    const ACCESS_CODE = Deno.env.get('CCAVENUE_ACCESS_CODE')?.trim();
    const WORKING_KEY = Deno.env.get('CCAVENUE_WORKING_KEY')?.trim();
    const REDIRECT_URL = Deno.env.get('CCAVENUE_REDIRECT_URL')?.trim();
    const CANCEL_URL = Deno.env.get('CCAVENUE_CANCEL_URL')?.trim();
    const CCAVENUE_URL = Deno.env.get('CCAVENUE_URL')?.trim() || 'https://test.ccavenue.com/transaction/transaction.do?command=initiateTransaction';

    // Log credential info for debugging (safe - only first/last chars)
    console.log('🔐 === CCAvenue Credential Check (GUEST) ===');
    ...
    if (!MERCHANT_ID || !ACCESS_CODE || !WORKING_KEY || !REDIRECT_URL || !CANCEL_URL) {
      ...
      return new Response(
        JSON.stringify({ error: 'CCAvenue configuration missing. Please configure all secrets.' }),
    ...
    console.log('✅ All CCAvenue credentials validated successfully');
    ...
    // Generate unique order ID for guest order
    // Format: GUEST_ORD_{timestamp}_{email_hash}
    const emailHash = email.substring(0, 8).replace(/[^a-z0-9]/gi, '');
    const orderId = `GUEST_ORD_${Date.now()}_${emailHash}`;
    ...
    console.log('✅ Guest order created in database:', orderId);
    ...
    console.log('📋 CCAvenue params prepared:', {
      merchant_id: MERCHANT_ID,
      order_id: orderId,
    ...
    console.error('❌ Error in create-guest-ccavenue-order:', error);
    return new Response(
