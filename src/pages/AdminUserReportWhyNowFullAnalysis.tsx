...
export default function AdminUserReportWhyNowFullAnalysis() {
  const navigate = useNavigate();
  const { id } = useParams();
  
  const { data: userReport, isLoading } = useQuery({
    queryKey: ['admin-user-report-why-now', id],
    queryFn: async () => {
      if (!id) return null;
      
      const { data: report, error: reportError } = await supabase
        .from('user_reports')
        .select('idea_title, why_now_full_analysis, why_now_score, news_data, user_id, created_at')
        .eq('id', id)
        .maybeSingle();
              ...
              <div className="space-y-4">
                {Array.isArray(userReport.news_data) && userReport.news_data.map((news: any, index: number) => (
                  <div key={index} className="border-b pb-4 last:border-b-0">
                    ...
                    {news.link && (
                      <a href={news.link} target="_blank" rel="noopener noreferrer" className="text-sm text-primary hover:underline">
                        Read more →
                      </a>
                    )}
                  </div>
