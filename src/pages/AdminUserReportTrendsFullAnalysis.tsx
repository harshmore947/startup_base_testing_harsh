...
export default function AdminUserReportTrendsFullAnalysis() {
  const navigate = useNavigate();
  const { id } = useParams();
  
  const { data: userReport, isLoading } = useQuery({
    queryKey: ['admin-user-report-trends', id],
    queryFn: async () => {
      if (!id) return null;
      
      const { data: report, error: reportError } = await supabase
        .from('user_reports')
        .select('idea_title, trends_full_analysis, trends_score, trends_data, trends_query, user_id, created_at')
        .eq('id', id)
        .maybeSingle();
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container max-w-6xl mx-auto px-4 py-8">
        ...
        {userReport.trends_data && (
          <Card className="mt-8">
            <CardHeader>
              <CardTitle>Trend Data Visualization</CardTitle>
              {userReport.trends_query && (
                <p className="text-sm text-muted-foreground">Search Query: {userReport.trends_query}</p>
              )}
            </CardHeader>
            <CardContent>
              <TrendChart data={userReport.trends_data} />
            </CardContent>
          </Card>
        )}
      </div>
