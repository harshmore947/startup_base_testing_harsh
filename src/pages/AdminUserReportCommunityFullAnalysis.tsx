...
export default function AdminUserReportCommunityFullAnalysis() {
  const navigate = useNavigate();
  const { id } = useParams();
  
  const { data: userReport, isLoading } = useQuery({
    queryKey: ['admin-user-report-community', id],
    queryFn: async () => {
      if (!id) return null;
      
      const { data: report, error: reportError } = await supabase
        .from('user_reports')
        .select('idea_title, community_signal_full_analysis, community_signal_score, youtube_data, reddit_data, youtube_query, reddit_query, user_id, created_at')
        .eq('id', id)
        .maybeSingle();
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container max-w-6xl mx-auto px-4 py-8">
        ...
        <Tabs defaultValue="youtube" className="mt-8">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="youtube">YouTube Discussions</TabsTrigger>
            <TabsTrigger value="reddit">Reddit Discussions</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="reddit">
            {userReport.reddit_data && (
              <RedditDiscussions 
                redditData={userReport.reddit_data}
              />
            )}
          </TabsContent>
        </Tabs>
