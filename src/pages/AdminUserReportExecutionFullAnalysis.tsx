...
export default function AdminUserReportExecutionFullAnalysis() {
  const navigate = useNavigate();
  const { id } = useParams();
  
  const { data: userReport, isLoading } = useQuery({
    queryKey: ['admin-user-report-execution', id],
    queryFn: async () => {
      if (!id) return null;
      
      const { data: report, error: reportError } = await supabase
        .from('user_reports')
        .select('idea_title, execution_plan, user_id, created_at')
        .eq('id', id)
        .maybeSingle();
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container max-w-6xl mx-auto px-4 py-8">
        <InteractiveExecutionPlan 
          executionPlan={userReport.execution_plan} 
          showProgress={true}
        />
      </div>
    </div>
