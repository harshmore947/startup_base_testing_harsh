...
export default function AdminUserReportOpportunityFullAnalysis() {
  const navigate = useNavigate();
  const { id } = useParams();
  
  const { data: userReport, isLoading } = useQuery({
    queryKey: ['admin-user-report-opportunity', id],
    queryFn: async () => {
      if (!id) return null;
      
      const { data: report, error: reportError } = await supabase
        .from('user_reports')
        .select('idea_title, overall_full_analysis, opportunity_score, user_id, created_at')
        .eq('id', id)
        .maybeSingle();
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container max-w-6xl mx-auto px-4 py-8">
        <InteractiveOpportunityAnalysis 
          content={userReport.overall_full_analysis} 
          score={userReport.opportunity_score}
        />
      </div>
    </div>
  );
