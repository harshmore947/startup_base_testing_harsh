...
export default function AdminUserReportRevenueFullAnalysis() {
  const navigate = useNavigate();
  const { id } = useParams();
  
  const { data: userReport, isLoading } = useQuery({
    queryKey: ['admin-user-report-revenue', id],
    queryFn: async () => {
      if (!id) return null;
      
      const { data: report, error: reportError } = await supabase
        .from('user_reports')
        .select('idea_title, revenue_plan, user_id, created_at')
        .eq('id', id)
        .maybeSingle();
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container max-w-6xl mx-auto px-4 py-8">
        <AnalysisContent 
          content={typeof userReport.revenue_plan === 'string' ? userReport.revenue_plan : JSON.stringify(userReport.revenue_plan, null, 2)} 
          score={null}
        />
      </div>
    </div>
